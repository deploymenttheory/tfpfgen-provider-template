...
Every setting below is shown as working providers actually set it, because the
comparison says more than any one of them alone:

- **ThousandEyes** — hosted SaaS. A static token, one address for everybody,
  account-scoped.
- **Jamf Pro** — self-hosted software. An OAuth grant, a different address for
  every customer, no scoping concept.
- **GitHub** — hosted SaaS at a scale that changes the answers. 808 paths, a
  description that declares no authentication at all, and objects addressed by
  name rather than by id.

Nothing in this file is shaped around any one of them.

| Setting | ThousandEyes | Jamf Pro | GitHub |
|---|---|---|---|
| `provider.name` | `thousandeyes` | `jamfpro` | `github` |
| `provider.displayName` | `ThousandEyes` | `Jamf Pro` | `GitHub` |
| `provider.apiEndpoint` | `https://api.thousandeyes.com/v7` | *(none — self-hosted)* | `https://api.github.com` |
...
Read across the rows and you have the whole design: an API that issues tokens
against one that grants them, an address everybody shares against one per
customer, an API that scopes by account against two that do not, and a
description that documents its own authentication against one that does not
bother. All three are generated by the same pipeline from the same file.
...
Either is fine, and so is either format: a description may be YAML or JSON,
and GitHub's is JSON. What matters is that it is reachable and stays put.
...
**GitHub is where `includeOnlyPaths` stops being optional.** Its description
has 808 paths covering Actions, Codespaces, Copilot, Dependabot, Gists,
Packages, Projects and much else. Generating all of it produces thousands of
files and a provider nobody wants, and every regeneration takes minutes. A
provider is usually about a handful of things:
...
Scope first, widen later. Narrowing costs one line here; discovering afterwards
that you generated 800 paths costs an afternoon.
...
**Some descriptions declare no authentication at all**, and GitHub's is one:
